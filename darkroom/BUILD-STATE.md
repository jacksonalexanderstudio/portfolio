...
**Orb video placeholder** — YouTube embed iframe inside a `<foreignObject>` (480×320, 3:2) positioned between the planet body and front rings. The SVG has `overflow:visible` so Chrome doesn't clip the iframe at the SVG CSS box. A black `<circle r="80">` mask sits on top of the iframe until the YouTube IFrame API confirms playback has actually started (polls `getCurrentTime()` ticks), at which point the mask gets `display:none` and the video is revealed. Hides the buffering/pause overlay during the initial seek to `start=1200`. 1.4× zoom on the video element crops out YouTube chrome bleed. Video is locked to the placeholder for the duration of the session — tab-capture audio still routes through the engine but the orb visual doesn't swap.
...
- **Decisions still on the table:**
  ...
  - "RELEASE" verb vs "BYPASS" — release is overloaded with envelope-release in audio.
